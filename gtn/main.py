...
X = torch.from_numpy(X.reshape(X.shape[0], dim_timestep, dim_feature))
...
for e in tqdm(range(1, EPOCHS + 1), bar_format="{l_bar}{bar:10}{r_bar}{bar:-10b}"):
    ...
    for X_train_batch, y_train_batch in train_loader:
        ...
        y_train_pred = model(X_train_batch, 'train')
    ...
    with torch.no_grad():
        ...
        for X_val_batch, y_val_batch in val_loader:
            ...
            y_val_pred = model(X_val_batch, 'test')
...
with torch.no_grad():
    ...
    for X_batch, _ in test_loader:
        ...
        y_test_pred = model(X_batch, 'test')
